...
do_load:
    ...
    push ax                      ; keep the target length safe across the INT 13h call below
    mov cx, ax
    add cx, 511
    shr cx, 9
    mov word [dap_count], cx
    ...
    int 0x13
    pop ax
    jc .err                      ; read failed - buf_len left alone, buffer stays consistent

    mov [buf_len], ax            ; only commit the new length once the data actually landed
    jmp .skip_data
